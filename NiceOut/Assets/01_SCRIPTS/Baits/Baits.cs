...
public class Baits : MonoBehaviour // detail d'achat et d'upgrade des pieges
{
    ...
    private void Start()
    {
        ...
        previewTimer.GetComponent<Bait_Preview_Timer>().trap = this;
    ...
    private void Update()
    {
        ...
        if (isPaused == false)
        {
            ...
            if (cooldownCountdown <= 0 && hasSPawned == false)
            {
                ...
                this.child.GetComponent<Bait_Attack>().parentTrap = this.gameObject;
                this.child.GetComponent<Bait_Attack>().type = this.trapType;
                this.child.GetComponent<Bait_Attack>().canAttack = true;
                ...
                hb.GetComponent<Bait_Stats>().trap = this;
                hb.GetComponent<Bait_Stats>().player = this.player;
            ...
            if (cooldownCountdown <= 0 && hasSPawned == true)
            {
                ...
                if (this.usure <= 1f)
                {
                    child.GetComponent<Bait_Attack>().isGonnaDie = true;
                }
                if (this.usure <= 0)
                {
                    this.location.GetComponent<Bait_Location_State>().SetOccupation(false);
                    Destroy(this.child);
